...
func (a *Allocator) allocateNode(ctx context.Context, node *api.Node, existingAddressesOnly bool, networks []*api.Network) bool {
	...
	for _, network := range networks {
		//TODO: Is there a better way to determine if a network is global?
		if network.DriverState.Name != "overlay" {
			continue
		}
		...
		if err := a.netCtx.nwkAllocator.AllocateLBAttachment(node, network.ID); err != nil {
			...
			//TODO: Should we fail here instead and deallocate instead?
			//If we there are multiple networks, and we faild to allocate a LB on that network,
			//we could return true and commit the node.
			continue
		}
