...
type NetworkAllocator interface {
	...
	// DeallocateLBAttachment Deallocates a load balancer endpoint
	DeallocateLBAttachment(node *api.Node, nid string) error

	//IsLBAttachmentAllocated If lb endpoint is allocated on the node
	IsLBAttachmentAllocated(node *api.Node, nid string) bool
}
